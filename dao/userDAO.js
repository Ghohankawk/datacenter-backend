...
module.exports = {
    add: function (user, callback) {
        pool.query(userSqlMap.add, [user.name, user.password, user.age], function (error, result) {
    ...
    update: function (user, callback) {
        pool.query(userSqlMap.update, [user.name, user.password, user.age, user.id], function (error, result) {
            if (error) throw error;
            callback(result.affectedRows > 0);
        });
    }
